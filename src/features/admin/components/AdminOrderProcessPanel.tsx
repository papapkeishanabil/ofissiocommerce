  ...
  function startProcess() {
    ...
    startTransition(async () => {
      ...
      const result = (await response.json().catch(() => ({}))) as {
        ok?: boolean;
        message?: string;
        order?: { processStatus?: string };
      };
      if (!response.ok || !result.ok) {
        setMessage(result.message ?? "Order belum dapat diproses.");
        return;
      }
      setMessage(`Route ${processRouteLabel(processRoute)} masuk status ${result.order?.processStatus ?? "in_progress"}.`);
      router.refresh();
    });
  }
    ...
    <section className="rounded-3xl border border-line bg-surface p-5 shadow-soft-sm">
      ...
      <div className="mt-4 rounded-2xl bg-slate-50 p-4">
        ...
        <ol className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, index) => (
            <li key={step} className="rounded-2xl bg-white p-3 text-sm ring-1 ring-line">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button
          ...
          disabled={isPending || isCompleted || isInProgress}
          aria-busy={isPending}
        >
          {isPending
            ? "Memproses..."
        ...
        <p className="self-center text-xs font-semibold text-ink-muted">
          Production Order detail penuh disiapkan untuk Phase 19.
        </p>
      </div>
    </section>
  );
