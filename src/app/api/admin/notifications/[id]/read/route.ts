...
import { markNotificationRead } from "@/features/admin-notifications/admin-notification.service";
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    rateLimitOrThrow({ key: createRateLimitKey(request, "admin.notifications.read"), limit: 80, windowMs: 60_000 });
    ...
    const notification = await markNotificationRead(id, { actorId: actor.id, request });
    if (!notification) throw createApiError("NOT_FOUND", "Notifikasi tidak ditemukan.", 404);
    return NextResponse.json({ ok: true, notification });
  } catch (error) {
    return safeErrorResponse(error, "Notifikasi belum dapat ditandai dibaca.", 403);
  }
}
