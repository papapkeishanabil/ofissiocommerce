...
-- Task G1 revision: Ginee inventory read-only stock checker metadata.
create table if not exists public.ginee_product_mappings (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ginee_product_mappings_inventory_readonly check (sync_stock_enabled = false)
);
...
create index if not exists ginee_inventory_snapshots_stock_sku_idx on public.ginee_inventory_snapshots (stock_sku, checked_at desc);
create index if not exists ginee_inventory_snapshots_ginee_sku_idx on public.ginee_inventory_snapshots (ginee_sku, checked_at desc);
create index if not exists ginee_inventory_snapshots_mapping_idx on public.ginee_inventory_snapshots (mapping_id, checked_at desc);
...
-- WooCommerce stock monitoring: internal replenishment request queue.
